...
export const zh = {
  ...
  'custom.history.keepRecentTurns': '保护近期轮数',
  'custom.history.keepRecent': '保护近期窗口',
  ...
  'custom.tailTrim.warning': 'TailTrim 只在精确 tokenizer 可用时，把一个完整、已结束且非错误的纯工具组替换为可恢复引用；它与 History 共用近期 turn、近期窗口和最小回收参数。它会改写已发送前缀，可能降低缓存命中。',
...
export const en = {
  ...
  'custom.tailTrim.warning': 'TailTrim requires the exact tokenizer and replaces at most one complete, finished, non-error tool-only group with a recoverable reference. It shares Protected recent turns, Protected recent window, and Minimum reclaim with History. It rewrites a sent prefix and may reduce cache hits.',
