...
- `autoCompact.thresholdPercent` setting (default 80, integer 50–90, step 1) with one shared validation contract across the settings UI, the persisted schema, and the runtime resolver. The editor — number input, slider, and 70/80/85 quick values on one draft with risk notes — lives inside the context-compression selector settings section.
